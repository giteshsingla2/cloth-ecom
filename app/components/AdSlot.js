...
export default function AdSlot({ type = 'banner', className = '', seed = 0 }) {
  ...
  useEffect(() => {
    setMounted(true);
    setRandomIdx(Math.floor(Math.random() * (products.length || 1)));
  }, [products.length]);

  if (!products.length) return null;

  // Use seed-based index for SSR so server HTML matches client hydration HTML
  const targetProduct = mounted ? products[randomIdx] : products[seed % products.length];
  const targetHeadline = mounted ? adHeadlines[randomIdx % adHeadlines.length] : adHeadlines[seed % adHeadlines.length];

  if (type === 'banner') {
    return (
      <div className={`fake-ad-banner ${className}`}>
        <Link href={`/product/${targetProduct.id}`} className="ad-banner-link">
          <span className="ad-badge">SPONSORED AD</span>
          <div className="ad-banner-content">
            <span className="ad-flash-dot"></span>
            <p className="ad-text">{targetHeadline}</p>
  ...
  if (type === 'sidebar') {
    return (
      <div className={`fake-ad-sidebar ${className}`}>
        ...
        <Link href={`/product/${targetProduct.id}`} className="ad-sidebar-link">
          <div className="ad-image-container">
            <img src={targetProduct.image} alt="Sponsored Ad" className="ad-sidebar-img" />
          ...
          <div className="ad-sidebar-body">
            ...
            <p className="ad-sidebar-text">{targetProduct.title.slice(0, 50)}...</p>
            <div className="ad-sidebar-price">
              <span className="old-price">₹{targetProduct.originalPrice}</span>
              <span className="new-price">₹{targetProduct.price}</span>
            </div>
  ...
  if (type === 'native-grid') {
    // Select 3 deterministic products during SSR, randomized after mount
    const items = mounted
      ? [...products].sort(() => 0.5 - ((seed + randomIdx) % 3 ? 0.1 : 0.9)).slice(0, 3)
      : products.slice(seed % products.length, (seed % products.length) + 3);
